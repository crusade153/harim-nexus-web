...
import { Archive, Link as LinkIcon, ExternalLink, MessageSquare, Plus, X, AlignLeft, Tag, Send } from 'lucide-react'
import Editor from '@/components/ui/Editor'
import { createArchive, createComment } from '@/lib/sheets'
    ...
    <div className="h-full flex flex-col md:flex-row gap-6">
      <div className="w-full md:w-80 flex flex-col gap-4 border-r border-slate-200 dark:border-slate-700 pr-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setIsModalOpen(true)} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-500 transition-colors"><Plus size={20} /></button>
        </div>
              ...
              <div className="space-y-4 mb-6">
                {selectedDoc.댓글?.map((cmt, idx) => (
                  <div key={idx} className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-500">{cmt.작성자[0]}</div>
                    <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl rounded-tl-none"><div className="flex items-center gap-2 mb-1"><span className="text-xs font-bold text-slate-700 dark:text-slate-300">{cmt.작성자}</span><span className="text-[10px] text-slate-400">{cmt.시간}</span></div><p className="text-sm text-slate-600 dark:text-slate-400">{cmt.내용}</p></div>
                  </div>
                ))}
              </div>
              <div className="flex gap-2">
                <input type="text" value={commentInput} onChange={(e) => setCommentInput(e.target.value)} placeholder="댓글을 입력하세요..." className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white" onKeyDown={e => e.key === 'Enter' && handleAddComment()} />
                <button onClick={handleAddComment} className="btn-primary py-2 text-xs px-4"><Send size={14}/></button>
              </div>
            </div>
          </div>
        ) : (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl relative flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700"><h3 className="text-lg font-bold text-slate-900 dark:text-white">새 지식 추가</h3><button onClick={() => setIsModalOpen(false)}><X size={20}/></button></div>
            ...
            <div className="flex gap-3 p-6 pt-2 border-t"><button onClick={() => setIsModalOpen(false)} className="flex-1 btn-secondary">취소</button><button onClick={handleSave} className="flex-1 btn-primary">저장</button></div>
          </div>
        </div>
      )}
    </div>
