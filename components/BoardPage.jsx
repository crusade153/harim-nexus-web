...
import { createPost } from '@/lib/sheets' // ✅ [New] 저장 함수 불러오기
            ...
            <div key={i} className="card-base p-6 flex flex-col justify-between group cursor-pointer hover:border-blue-300 dark:hover:border-blue-500 transition-colors">
              <div>
                <div className="flex justify-between items-start mb-4">
                  <span className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${getTagStyle(post.태그)}`}>{post.태그}</span>
                  <span className="text-xs text-gray-400">{post.날짜}</span>
                </div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 leading-snug mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{post.제목}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2">{post.내용}</p>
              </div>
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-[10px] font-bold dark:text-white">{post.작성자명[0]}</div>
            ...
            <tbody className="text-sm">
              {filteredPosts.map((post, i) => (
                <tr key={i} className="group border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                  ...
                  <td className="py-4 pr-4"><span className="font-medium text-gray-900 dark:text-gray-200 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors block truncate max-w-lg">{post.제목}</span></td>
                  <td className="py-4"><div className="flex items-center gap-2"><div className="w-6 h-6 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center text-[10px] text-gray-600 dark:text-gray-300 font-bold">{post.작성자명[0]}</div><span className="text-gray-600 dark:text-gray-400">{post.작성자명}</span></div></td>
                  <td className="py-4 text-gray-500 dark:text-gray-500 text-xs">{post.날짜}</td>
                  <td className="py-4 text-center"><span className="text-xs text-gray-400 bg-gray-50 dark:bg-slate-800 px-2 py-1 rounded-lg">{post.댓글}</span></td>
                </tr>
              ))}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-2xl shadow-2xl relative">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input type="text" value={newPost.제목} onChange={(e) => setNewPost({...newPost, 제목: e.target.value})} className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition-all dark:text-white" placeholder="제목을 입력하세요" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">카테고리</label>
                  <select value={newPost.태그} onChange={(e) => setNewPost({...newPost, 태그: e.target.value})} className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border border-transparent focus:border-indigo-500 outline-none dark:text-white appearance-none">
                    <option>일반</option><option>이슈</option><option>공지</option><option>긴급</option><option>자료</option>
                  </select>
                </div>
                <div>
              ...
              <div>
                ...
                <textarea value={newPost.내용} onChange={(e) => setNewPost({...newPost, 내용: e.target.value})} className="w-full h-40 px-4 py-3 bg-gray-50 dark:bg-slate-800 rounded-xl border border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none resize-none transition-all dark:text-white" placeholder="내용을 입력하세요..." />
              </div>
            </div>
