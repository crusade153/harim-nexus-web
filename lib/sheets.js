...
export async function getRealData() {
  try {
    ...
    // ✅ [수정] 업무(Tasks)에도 댓글 연결! (이 부분이 핵심입니다)
    const tasks = (tasksRes.data || []).map(t => {
      // 해당 업무 ID(post_id)를 가진 댓글 필터링
      const taskComments = allComments.filter(c => c.post_id === t.id).map(c => ({
        작성자: c.author_name, 
        내용: c.content, 
      ...
      return {
        ...
        마감일: t.due_date || '', 
        내용: t.content || '', 
        댓글: taskComments // ✅ 이제 DB 댓글이 여기에 들어갑니다
      }
    })
    ...
    // 5. 팀원 (Members) - 유경덕/관리자 맨 앞 정렬 유지
    ...
    return { currentUser, members, tasks, projects, archives, posts, schedules, holidays, quickLinks, activities }
...
async function logActivity(userName, action) {
  await supabase.from('activities').insert([{ user_name: userName || '알 수 없음', action }])
}
...
export async function createArchive(newDoc) {
  const { error } = await supabase.from('archives').insert([{
    category: newDoc.카테고리, title: newDoc.제목, link: newDoc.링크, content: newDoc.내용, author: newDoc.작성자
  }])
  if (error) throw error
...
export async function createProject(newProject) {
  const { error } = await supabase.from('projects').insert([{
    title: newProject.제목, author: newProject.작성자, period: newProject.기간
  }])
  if (error) throw error
...
export async function createTodo(newTodo) {
  const { error } = await supabase.from('todos').insert([{
    project_id: Number(newTodo.projectID), content: newTodo.항목, assignee: newTodo.담당자, is_done: false
  }])
  if (error) throw error
}
...
export async function createTask(newTask) {
  const { error } = await supabase.from('tasks').insert([{
    title: newTask.제목, status: '대기', priority: newTask.우선순위 || '보통', assignee: newTask.담당자명, due_date: newTask.마감일, content: newTask.내용
  }])
  if (error) throw error
...
export async function createSchedule(newSchedule) {
  const { error } = await supabase.from('schedules').insert([{
    type: newSchedule.유형, sub_type: newSchedule.세부유형, content: newSchedule.내용, date: newSchedule.날짜, time: newSchedule.시간, target: newSchedule.대상자
  }])
  if (error) throw error
  await logActivity('팀원', `님이 캘린더에 [${newSchedule.내용}] 일정을 등록했습니다.`)
}

// ✅ [중요] 업무 상태 변경 (드래그 앤 드롭 저장용)
