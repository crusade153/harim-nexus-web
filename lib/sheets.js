  ...
  try {
    ...
    const tasks = (tasksRes.data || []).map(t => {
      ...
      return {
        ...
        마감일: t.due_date || '', 
        내용: t.content || '', 
        관련문서ID: t.related_doc_id || null, // ✅ [추가] DB 컬럼명 확인 필요 (여기선 related_doc_id로 가정)
        댓글: taskComments 
      }
    })
...
export async function updatePost(postId, updatedData) {
  ...
  if (error) throw error
  await logActivity('사용자', `님이 게시글 [${updatedData.제목}]을 수정했습니다.`)
}
...
// ✅ [추가] 아카이브 수정
export async function updateArchive(docId, updatedDoc) {
  ...
  if (error) throw error
  await logActivity('사용자', `님이 지식고 문서 [${updatedDoc.제목}]을 수정했습니다.`)
}
...
export async function updateProject(projectId, updatedData) {
  ...
  if (error) throw error
  await logActivity('사용자', `님이 프로젝트 [${updatedData.제목}]을 수정했습니다.`)
}
...
// ✅ [추가] 업무 정보 수정 (내용, Wiki연결 등)
export async function updateTask(taskId, updates) {
  // DB 컬럼 매핑: related_doc_id가 DB 컬럼이라고 가정
  const dbUpdates = {}
  ...
  if(updates.관련문서ID) dbUpdates.related_doc_id = Number(updates.관련문서ID) // 숫자형 변환
