...
// 9. 현재 사용자 (진짜 로그인 정보 가져오기)
const { data: { user } } = await supabase.auth.getUser()
let currentUser = null

if (user) {
  // 로그인한 유저의 Auth ID로 members 테이블에서 정보 찾기
  const member = members.find(m => m.ID === user.id || m.아이디 === user.email?.split('@')[0] || m.이메일 === user.email)
  
  // 만약 members 테이블에서 못 찾으면 기본 정보라도 채워넣기
